...
/**
 * Simple Adapter for the MXA Chat Items. Takes the items from the database
 * and does a simple layout:
 * ----------------------
 * -me:Hi John-----------
 * -(date)---------------
 * ----------------------
...
public class ChatAdapter extends SimpleCursorAdapter{
	 ...
	 * @param xmppid The XMPP ID of the current user
	 */
	...
	/**
	 * Creates the View for the ChatAdapter.
	 * This methode filters the xmpp id and gives the own messages a slight
	 * red background, and all others a slightly yellow one.
	 * Also generates a date out of the timestamp
	 * @param v	View to be used
	 * @param c Cursor the database with messages
	 * @return the generated View
	 */
	public View create(View v,Cursor c)
	{
		TextView tv=(TextView)v.findViewById(R.id.chat_item_text_view);
		Date sendTime=new Date(Long.valueOf(c.getString(c.getColumnIndex(MessageItems.DATE_SENT))));
		String date;
...
//		if (sender!=null) sender= sender.split("@")[0];
		...
		if (mUserXMPPId!=null)
		{
			if (sender.contains(mUserXMPPId.split("/")[0]))
			{
				...
				if (sender.contains("@")) sender=sender.split("@")[0];
		...
		tv.setText(Html.fromHtml("<b>"+sender+":</b>"+body+"<br>("+date+")"));
		tv.setTextColor(Color.BLACK);
		
		//generate the greyish pattern for the date string
		Spannable s=(Spannable) tv.getText();
		s.setSpan(new TextAppearanceSpan(context, android.R.style.TextAppearance_Small),
				tv.getText().toString().indexOf(date)-1 ,
				date.length() +tv.getText().toString().indexOf(date)+1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		s.setSpan(new ForegroundColorSpan(Color.parseColor("#9f9f9f")),
				tv.getText().toString().indexOf(date)-1 ,
				date.length() +tv.getText().toString().indexOf(date)+1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
