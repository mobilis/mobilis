var xhunt = {
	...
	addHandlers: function () {
		...
		console.log('hier, handler gesetzt');
	...
	on_joinGameIQ: function (iq) {
		...
		if (xhunt.gameinfo['joined'] === 'false'){
			...
			Mobilis.connection.muc.join(
				...
				xhunt.gameinfo['nick'],
				function(message) {
					console.log('bin jetz  in der lobby');
		...
		Mobilis.core.mobilisServiceDiscovery(
			...
			function (iq) {
				...
				if ($(iq).find("mobilisService").length){
					...
					gameList.append('<li data-role="divider" data-theme="a">Available Games</li>');
					...
					$(iq).find("mobilisService").each(function() {
						...
						gameList.append('<li><a class="available-game" id="' + $(this).attr('jid') + '" href="#" data-transition="slide" data-rel="back">' + $(this).attr('serviceName') + '</a></li>');
					}); 
				} else {
...
$(document).on('joinGame',
	function(ev, data) {
		...
		console.log('data.jid: '+data.jid);

		Mobilis.xhunt.joinGame(
			data.jid, //gameJID
			'XHunt WebClient', //playerName
			function (iq){  // resultcallback
				...
				//xhunt.addHandlers();
				console.log('hier jetz addhandlers');
			},
